...
public class BNavigatorActivity extends Activity{
	...
	public void onCreate(Bundle savedInstanceState){
		...
		BNavigator.getInstance().setListener(mBNavigatorListener);//添加导航监听器
		BNavigator.getInstance().startNav();//启动导航		
		// 初始化TTS. 开发者也可以使用独立TTS模块，不用使用导航SDK提供的TTS
		BNTTSPlayer.initPlayer();//初始化TTS播放器
		//设置TTS播放回调
		BNavigatorTTSPlayer.setTTSPlayerListener(new IBNTTSPlayerListener() {
            ...
            @Override
            public int getTTSState() {// 获取TTS当前播放状态,返回0则表示tts不可用
	...
	private IBNavigatorListener mBNavigatorListener = new IBNavigatorListener() {
		...
		@Override
		public void notifyStartNav() {//开始导航
			// TODO Auto-generated method stub
			BaiduNaviManager.getInstance().dismissWaitProgressDialog();//关闭等待对话框
		}
    ...
    @Override
    public void onConfigurationChanged(Configuration newConfig) {//当屏幕方向改变时或者系统设置改变时调用
    	BNavigator.getInstance().onConfigurationChanged(newConfig);
        super.onConfigurationChanged(newConfig);
    }
    
    public void onBackPressed(){//点击返回键
